...
def save_timestamps(participant_num, timestamps):
    with open(f'timestamps_{participant_num}.csv', 'w', newline='') as csvfile:
        ...
        writer.writerow(['Stage', 'Timestamp'])
        for stage, ts in timestamps.items():
            writer.writerow([stage, f"{ts:.4f}"])
            ...
            if past_instructions:
                if page_num >= global_vars.NUM_PAGES * global_vars.NUM_ROUNDS :
                    ...
                    if event.type == pygame.KEYDOWN and event.key == K_RETURN:
                        page_num += 1
                        now = time.time()
                        timestamps[f"dots_display_round_{page_num}"] = now - stage_start_time
                        beep()
                    ...
                    if event.type == pygame.KEYDOWN and event.key == K_RETURN:
                        page_num += 1
                        now = time.time()
                        timestamps[f"map_display_round_{page_num}"] = now - stage_start_time
                        beep()
                        stage_start_time = now
                    screen.blit(instructions_3, (0, 0))
                    ...
                    if event.type == pygame.KEYDOWN and event.key == K_RETURN:
                        ...
                        click_grid.tally()
                        now = time.time()
                        timestamps[f"dots_recall_round_{page_num}"] = now - stage_start_time
                        beep()
                        stage_start_time = now
